			...
			if ($user->rights->shipmentpackage->shipmentpackage->write && $object->statut == Expedition::STATUS_VALIDATED) {
				...
				if (is_array($result) && count($result) > 0) {
					foreach ($result as $package) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . $href . '?origin=shipping&id=' . $package->id . '&originid=' . $object->id . '&fk_soc=' . $object->socid . '&fk_project=' . $object->fk_project . '&action=addto&token=' . newToken() . '">' . $langs->trans('AddToPackage', $package->ref) . '</a></div>';
					}
				}
				print '<div class="inline-block divButAction"><a class="butAction" href="' . $href . '?origin=shipping&originid=' . $object->id . '&fk_soc=' . $object->socid . '&fk_project=' . $object->fk_project . '&action=create&token=' . newToken() . '">' . $langs->trans('CreatePackage') . '</a></div>';
			}
		}
		return $error;
	}
